...
nvflash () {
	if [ -z $1 ] ; then
		sudo ./utils/nvflash_v1.13.87205_miniloader_patched --setbct --bct ./bct/surface_rt_bct.BIN --configfile ./utils/flash.cfg --bl ./u-boot/apx-u-boot-dtb-tegra.bin --go
	else
		sudo ./utils/nvflash_v1.13.87205_miniloader_patched --setbct --bct ./bct/surface_rt_bct.BIN --configfile ./utils/flash.cfg --bl $1 --go
	fi
}
