...
copy () {
	chmod +x isowrt.sh
	if [[ $1 == "system" ]] ; then
		sudo rm /usr/local/bin/isowrt # Removing previous installation
		sudo cp isowrt.sh /usr/bin/local/isowrt
		...
		rm ~/.local/bin/isowrt # Removing previous installation
		cp isowrt.sh ~/.local/bin/isowrt
